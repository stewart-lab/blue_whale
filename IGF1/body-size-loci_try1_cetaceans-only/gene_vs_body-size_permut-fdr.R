...
compute_pvals <- function(alignment_matrix, my_species, body_mass, bw_start_col, 
  ...
  for (i in bw_start_col:bw_stop_col) {
    ...
    if (length(base_counts) == 1 | sort(base_counts, decreasing = TRUE)[2] < 2) {
...
num_hits$num <- apply(num_hits, 1, function(x) sum(-log10(pvals) > x["threshold"]))
...
for (iteration in 1:opt$num_permutations) {
  ...
  resamp_body_mass$adult_body_mass <- sample(body_mass$adult_body_mass)
